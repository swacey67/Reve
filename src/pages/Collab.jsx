      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 lg:gap-24 w-full mt-12 md:mt-0">
        <div className="flex flex-col items-center group cursor-pointer" onClick={() => navigate('/collab/jjk')}>
          <div className="scale-110 mb-10 drop-shadow-2xl transition-transform duration-500 group-hover:scale-125 group-hover:-translate-y-4">
            <Tumbler patternId="jjkPattern" handleColor="#111111" headColor="#901A1E" lidBaseColor="#1A3E8A" isStatic={true} delay={100} />
          </div>
          <p className="font-sans font-bold text-[#F5F4EF] text-sm md:text-[1.05rem] tracking-wide transition-colors group-hover:text-white">Poignée X Jujutsu Kaisen</p>
        </div>

        <div className="flex flex-col items-center group cursor-pointer" onClick={() => navigate('/collab/vagabond')}>
          <div className="scale-110 mb-10 drop-shadow-2xl transition-transform duration-500 group-hover:scale-125 group-hover:-translate-y-4">
            <Tumbler patternId="vagabondPattern" handleColor="#F5F5F5" headColor="#E0E0E0" isStatic={true} delay={250} />
          </div>
          <p className="font-sans font-bold text-[#F5F4EF] text-sm md:text-[1.05rem] tracking-wide transition-colors group-hover:text-white">Poignée X Vagabond</p>
        </div>

        <div className="flex flex-col items-center group cursor-pointer" onClick={() => navigate('/collab/chainsaw')}>
          <div className="scale-110 mb-10 drop-shadow-2xl transition-transform duration-500 group-hover:scale-125 group-hover:-translate-y-4">
            <Tumbler patternId="collabPattern" handleColor="#1E195E" headColor="#1E195E" lidBaseColor="#1A3E8A" isStatic={true} delay={400} />
          </div>
          <p className="font-sans font-bold text-[#F5F4EF] text-sm md:text-[1.05rem] tracking-wide transition-colors group-hover:text-white">Poignée X Chainsaw Man</p>
        </div>
      </div>
    </div>
  );
